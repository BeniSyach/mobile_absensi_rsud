...
interface MasterData {
  ...
  responseData: (data: HargaKomoditiPasarRataRata[]) => void;
}

export const formScheme = z.object({
  pasar_id: z.string().min(1, 'Pasar wajib dipilih'),
  startDate: z.string().min(4, 'Tanggal Awal wajib diisi'),
  endDate: z.string().min(4, 'Tanggal Akhir wajib diisi'),
});

export default function FromInputPasar({
  Pasar,
  Komoditas,
  responseData,
}: MasterData) {
  ...
  const dataPasar: OptionType[] = Pasar.map((p) => ({
    label: p.nama,
    value: p.id,
  }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.get(
        'https://pihps.deliserdangkab.go.id/api/harga-komoditi-pasar',
        {
          params: {
  ...
  return (
    <View className="space-y-4">
      ...
      <Select
        label="Pasar"
        value={form.pasar_id}
        error={formErrors.pasar_id}
        onSelect={(opt) => handleChange('pasar_id', opt)}
        options={dataPasar}
