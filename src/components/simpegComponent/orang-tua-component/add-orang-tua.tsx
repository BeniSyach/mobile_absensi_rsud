...
import { FormOrangTua, type OrangTuaFormProps } from './form-orang-tua';

export default function AddOrangTua() {
  const router = useRouter();
  const { mutate, isPending, isError } = PostSPT();
  const onSubmit: OrangTuaFormProps['onSubmit'] = (data) => {
  ...
  return (
    <View className="mx-5 flex-row justify-start">
      <ButtonSecondary
        label="Tambah Data"
        renderForm={() => (
          <FormOrangTua
