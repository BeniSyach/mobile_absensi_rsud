...
import { type AciActivity, getAciActivities } from './aci-service';
  ...
  return (
    <View className="px-6 pt-6">
      <View className="flex-row items-center justify-between rounded-[40px] bg-[#0066FF] px-6 py-4 shadow-lg shadow-blue-200">
        ...
        <Text className="text-lg font-bold text-white">Aktivitas Saya</Text>
...
const ActivityItem = ({ item }: { item: AciActivity }) => {
  // Clean up potentially messy OS/Device strings
  const cleanOs = stripHtml(item.os)
    .replace(/^-+\s*/, '')
    .trim();
  ...
  // Combine readable parts
  const deviceString = [cleanOs, cleanDevice].filter(Boolean).join(' - ');
      ...
      <View className="mt-3 flex-row flex-wrap gap-2">
        ...
        {deviceString ? (
          <View className="flex-row items-center rounded-lg bg-gray-50 px-2 py-1">
            ...
            <Text
              className="ml-1 max-w-[200px] text-[10px] text-gray-600"
              numberOfLines={1}
            >
              {deviceString}
            </Text>
...
const useActivityLog = () => {
  const router = useRouter();
  const [activities, setActivities] = useState<AciActivity[]>([]);
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        ...
        const data = await getAciActivities(token);
        if (data && data.data) {
...
export default function AciActivityLog() {
